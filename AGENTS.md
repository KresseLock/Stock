...
3. **風控與壓力測試優化：** 這兩者與 `trading_sim.py` 的信心分數門檻相輔相成。本系統經過 6年 (2020-2026) 全週期壓力測試，調校出平衡穩健的黃金風控參數（5日均回報門檻 -1.0% / 上漲家數比例 30.0% / 個股停損 -8.0%），在大變動年份（如 2022 熊市）依然能有效保護資產（2022年回測報酬 `+10.16%` / MDD `-12.24%`），實現真正的絕對收益！
...
- **`scripts/optimize_factors.py`**: **特徵與超參數最佳化腳本**。使用 `Optuna` 框架，在不偷看未來的驗證集上，動態尋找勝率最高的技術指標參數組合並輸出 `best_factors.json`。
...
- **`trading_sim.py`**: **實戰級交易模擬回測器 (Out-of-Sample)**。
  ...
  - **動態參數覆蓋**：支援透過 CLI 參數（如 `--panic_ma5`, `--panic_breadth`, `--stop_loss` 等）直接覆蓋中央 `config.py` 設定，便於快速回測與網格搜尋。
...
- **`scripts/risk_stress_test.py`**: **風控參數多週期壓力測試與回測工具**。
  - 跨週期回測：整合回測歷史關鍵時期（2020新冠、2021牛市、2022熊市、2024牛市、2026震盪及6年大週期），藉此評估不同風控組合的穩定性，防止過度擬合。
  - 輸出壓力測試報告：自動將多方案績效矩陣寫入並更新 `reports/stress_test_report.md`。
...
8. **`reports/stress_test_report.md`**: 系統多週期壓力測試與風控評估報告。

---

## 4. 未來擴展方向與 AI 注意事項
1. **資料維護**: 若發現特徵檔中缺少某項資料，請優先檢查 `scraper.py` 的跳過機制，並利用 `scraper.py --check` 進行檢驗與修復。
2. **常數宣告限制**: 任何常數、參數或調參範圍變數，都必須宣告在 [config.py](file:///D:/VScode_Stock/Stock/config.py) 中，嚴禁在個別腳本中進行分散式寫死。
